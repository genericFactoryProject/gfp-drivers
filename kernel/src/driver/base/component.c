	...
	struct component *component)
{
	...
	if (!devres_open_group(adev->parent, adev, 0))
		return -ENOMEM;
...
static int component_match_realloc(struct component_match *match, size_t num)
{
	...
	new = kmalloc_array(num, sizeof(*new), 0);
	if (!new)
		return -ENOMEM;
	...
	void *data)
{
	 ...
	 */
	if (!devres_open_group(adev->parent, NULL, 0))
		return -ENOMEM;
	 ...
	 */
	if (!devres_open_group(component->dev, component, 0)) {
		devres_release_group(adev->parent, NULL);
		return -ENOMEM;
	}
	...
	int subcomponent)
{
	...
	component = kzalloc(sizeof(*component), 0);
	if (!component)
		return -ENOMEM;
