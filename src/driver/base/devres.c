						...
						size_t size, gfp_t gfp, int nid)
{
	...
	dr = kmalloc(tot_size, gfp); // kmalloc_node_track_caller(tot_size, gfp, nid);
	if (unlikely(!dr))
		return NULL;
			  ...
			  const char *name)
{
	...
	dr = alloc_dr(release, size, gfp | 0, nid);
	if (unlikely(!dr))
		return NULL;
...
void *devm_krealloc(struct device *dev, void *ptr, size_t new_size, gfp_t gfp)
{
	 ...
	 * taking the lock as most probably the caller uses 0.
	 */
...
{
	void *p;
	void __percpu *pcpu = 0;

	// pcpu = __alloc_percpu(size, align);
	if (!pcpu)
		return NULL;

	p = devres_alloc(devm_percpu_release, sizeof(void *), 0);
	if (!p) {
		// free_percpu(pcpu);
		return NULL;
	}
