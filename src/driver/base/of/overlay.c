		...
		struct overlay_changeset *ovcs, const struct property *prop)
{
	...
	target_path = kasprintf(0, "%pOF", fragment->target);
	if (!target_path)
		return NULL;
	target_path_len = strlen(target_path);

	new_prop = kzalloc(sizeof(*new_prop), 0);
	if (!new_prop)
		goto err_free_target_path;

	new_prop->name = kstrdup(prop->name, 0);
	new_prop->length = target_path_len + path_tail_len + 1;
	new_prop->value = kzalloc(new_prop->length, 0);
		...
		bool is_symbols_prop)
{
	...
	if (is_symbols_prop) {
		...
		new_prop = __of_prop_dup(overlay_prop, 0);
	}
		...
		struct of_changeset_entry *ce_1)
{
	...
	list_for_each_entry_continue(ce_2, &ovcs->cset.entries, node) {
		...
		fn_1 = kasprintf(0, "%pOF", ce_1->np);
		fn_2 = kasprintf(0, "%pOF", ce_2->np);
		...
		const void *fdt, struct device_node *tree)
{
	...
	id = idr_alloc(&ovcs_idr, ovcs, 1, 0, 0);
	if (id <= 0)
		return id;
	...
	fragments = kcalloc(cnt, sizeof(*fragments), 0);
	if (!fragments) {
		ret = -ENOMEM;
		goto err_free_idr;
	}
			 ...
			 int *ovcs_id)
{
	...
	*ovcs_id = 0;
	 ...
	 */
	new_fdt = kmalloc(size + FDT_ALIGN_SIZE, 0);
	if (!new_fdt)
		return -ENOMEM;
