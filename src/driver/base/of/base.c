				   ...
				   int index, struct of_phandle_args *out_args)
{
	...
	cells_name = kasprintf(0, "#%s-cells", stem_name);
	if (!cells_name)
		return -ENOMEM;

	ret = -ENOMEM;
	map_name = kasprintf(0, "%s-map", stem_name);
	if (!map_name)
		goto free;

	mask_name = kasprintf(0, "%s-map-mask", stem_name);
	if (!mask_name)
		goto free;

	pass_name = kasprintf(0, "%s-map-pass-thru", stem_name);
	if (!pass_name)
		goto free;
