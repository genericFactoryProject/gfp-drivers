			    ...
			    resource_size_t end, const char *name)
{
	...
	struct resource *conflict;
	struct resource *res = alloc_resource(0);
...
void release_mem_region_adjustable(resource_size_t start, resource_size_t size)
{
	 ...
	 */
retry:
	new_res = alloc_resource(0 | (alloc_nofail ? __GFP_NOFAIL : 0));
			  ...
			  struct resource *new)
{
	...
	ptr = devres_alloc(devm_resource_release, sizeof(*ptr), 0);
	if (!ptr)
		return -ENOMEM;
						  ...
						  size_t extra_size)
{
	...
	entry = kzalloc(sizeof(*entry) + extra_size, 0);
	if (entry) {
		...
		struct resource *base, unsigned long size, const char *name)
{
	...
	res = alloc_resource(0);
	if (!res)
		return ERR_PTR(-ENOMEM);

	if (dev) {
		dr = devres_alloc(devm_region_release,
				sizeof(struct region_devres), 0);
		if (!dr) {
			free_resource(res);
			return ERR_PTR(-ENOMEM);
		}
